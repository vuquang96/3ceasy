...
<div class="portfolio-shortcode-posts-filters" data-target-id="portfolio-posts-shortcode-id-<?php echo esc_attr( $atts['id'] ); ?>">
	<?php
...
<div id="portfolio-posts-shortcode-id-<?php echo esc_attr( $atts['id'] ); ?>" class="portfolio-posts-shortcode-alt <?php echo filter_var( $atts['display_preview'], FILTER_VALIDATE_BOOLEAN ) ? 'with-preview' : 'no-preview'; ?> grid style-<?php echo esc_attr( $atts['style'] ); ?> hover-style-<?php echo esc_attr( $atts['hover_effect'] ); ?>">

<?php if( $posts->have_posts() ): ?>

	<?php while ( $posts->have_posts() ): $posts->the_post(); ?>
	
		<?php get_template_part( 'template-parts/portfolio-alt' ); ?>
...
<?php if( isset( $atts['pagination']['enabled'] ) && filter_var( $atts['pagination']['enabled'], FILTER_VALIDATE_BOOLEAN ) && $posts->max_num_pages > 1 ): ?>
	...
	<?php if( $atts['pagination']['true']['pagination_style'] == 'ajax' ): ?>
		...
		<div class="ajax-pagination">
			<a
				href="javascript:;"
				data-target-id="#portfolio-posts-shortcode-id-<?php echo esc_attr( $atts['id'] ); ?>"
