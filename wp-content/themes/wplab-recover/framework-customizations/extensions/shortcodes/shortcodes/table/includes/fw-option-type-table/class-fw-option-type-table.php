...
class FW_Option_Type_Table extends FW_Option_Type
{
	...
	protected function _enqueue_static( $id, $option, $data ) {
		...
		wp_localize_script(
			...
			array(
				'msgEdit' => esc_html__( 'Edit', 'wplab-recover' ),
	...
	protected function _render( $id, $option, $data ) {
		...
		if ( ! $table_shortcode ) {
			trigger_error(
				esc_html__( 'table-builder option type must be inside the table shortcode', 'wplab-recover' ),
				E_USER_ERROR
			);
		}
	...
	protected function _get_defaults() {
		...
		return apply_filters( 'fw_option_type_table_defaults', array(
			'header_options'  => array(
				'table_purpose' => array(
					'type'    => 'select',
					'label'   => esc_html__( 'Table Styling', 'wplab-recover' ),
					'desc'    => esc_html__( 'Choose the table styling options', 'wplab-recover' ),
					'choices' => array(
						'pricing' => esc_html__( 'Use the table as a pricing table', 'wplab-recover' ),
						'tabular' => esc_html__( 'Use the table to display tabular data', 'wplab-recover' ),
					),
					'value'   => 'pricing',
					'attr'    => array(
						'data-allowed-rows' => json_encode( array(
								'pricing' => 'default-row heading-row pricing-row button-row desc-row',
								'tabular' => 'default-row heading-row'
							)
						),
						'data-allowed-cols' => json_encode( array(
							'pricing' => 'default-col highlight-col',
			...
			'row_options'     => array(
				'name' => array(
					...
					'desc'    => false,
					'choices' => array(
						'default-row' => esc_html__( 'Default row', 'wplab-recover' ),
						'heading-row' => esc_html__( 'Heading row', 'wplab-recover' ),
						'pricing-row' => esc_html__( 'Pricing row', 'wplab-recover' ),
						'button-row'  => esc_html__( 'Button row', 'wplab-recover' ),
						'desc-row'  => esc_html__( 'Description row', 'wplab-recover' )
					),
				)
			),
			'columns_options' => array(
				'name' => array(
					...
					'desc'    => false,
					'choices' => array(
						'default-col'   => esc_html__( 'Default column', 'wplab-recover' ),
						'desc-col'      => esc_html__( 'Description column', 'wplab-recover' ),
						'highlight-col' => esc_html__( 'Highlight column', 'wplab-recover' ),
						'center-col'    => esc_html__( 'Center text column', 'wplab-recover' )
					),
				)
			),
			'content_options' => array(
				...
				'pricing-row' => array(
					...
					'description' => array(
						...
						'value'        => '',
						'attr' => array(
							'placeholder' => esc_html__('per month', 'wplab-recover')
						),
						'wrapper_attr' => array(
							'class' => 'fw-col-sm-6'
						)
				...
				'button-row'  => array(
					'button' => ($button = fw()->extensions->get( 'shortcodes' )->get_shortcode( 'button' ))
						? array(
							'type'          => 'popup',
							'popup-title'   => esc_html__( 'Button', 'wplab-recover' ),
							'button'        => esc_html__( 'Add', 'wplab-recover' ),
